...
from .utils import load_weights_from_npz, collect_act, hist_act
...
class Attention_ViT(nn.Module):
    ...
    def __init__(self,
        ...
        self.handle = self.register_forward_hook(hist_act)
        self.inputs = []
        self.outputs = []
        self.output = None
    ...
    def forward(self, x):
        ...
        attn_weights = self.log_int_softmax(attn, self.qact_attn1.quantizer.scale)
        ...
        x = self.qact3(x)
        x = self.proj_drop(x)
        if self.calibrate_stage2:
            if self.feature_maps_out is None:
                self.feature_maps_out = [x.cpu()]
            else:
                self.feature_maps_out.append(x.cpu())
        else:
            self.feature_maps_out = None
...
class Block(nn.Module):
    ...
    def forward(self, x, last_quantizer=None):
        if isinstance(self.norm1, QIntLayerNorm):
            n1 = self.norm1(x, last_quantizer,
                                      self.qact1.quantizer)
        else:
            n1 = self.norm1(x)

        out = self.attn(self.qact1(n1))
        x = self.qact2(x + self.drop_path(out))
        
        if isinstance(self.norm2, QIntLayerNorm):
            n2 = self.norm2(x, self.qact2.quantizer,
        ...
        x = self.qact4(x + self.drop_path(
            self.mlp(
                self.qact3(n2))))
        return x, self.qact4.quantizer


class VisionTransformer(BaseQuant):
    """Vision Transformer
    ...
    def forward_features(self, x):
        ...
        for i, blk in enumerate(self.blocks):
            last_quantizer = self.qact1.quantizer if i == 0 else prev_quantizer
            x, prev_quantizer = blk(x, last_quantizer)

        if isinstance(self.norm, QIntLayerNorm):
            x = self.norm(x, prev_quantizer,
