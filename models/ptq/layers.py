...
class QConv2d(nn.Conv2d):
    ...
    def __init__(self,
        ...
        self.quantizer = build_quantizer(self.quantizer_str, self.bit_type,
                                         self.observer, self.module_type, self.bcorr_weights)
...
class QAct(nn.Module):
    ...
    def forward(self, x, prev_layer=None, x_prev=None):
        if self.calibrate:
            self.quantizer.observer.update(x)
            if self.last_calibrate:
                if self.observer_str == "joint":
                    self.quantizer.update_quantization_params(x, prev_layer, x_prev)
                else:
...
class QIntLayerNorm(nn.LayerNorm):
    ...
    def forward(self,
        ...
        if self.mode == 'ln':
            ...
            in_scale = in_scale.to(x.device)
            out_scale = out_scale.to(x.device)
