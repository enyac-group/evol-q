...
class OmseObserver(BaseObserver):
    ...
    def get_quantization_params(self, inputs):
        ...
        for i in range(90):
            ...
            new_scale = torch.ones_like(max_val, dtype=torch.float32)
            new_zero_point = torch.zeros_like(max_val, dtype=torch.int64)
            ...
            new_scale = new_scale.reshape(range_shape)
            new_zero_point = new_zero_point.reshape(range_shape)
            try:
                inputs_q = inputs / new_scale
            except:
                import pdb
                pdb.set_trace()
            inputs_q += new_zero_point
            inputs_q = torch.round(inputs_q)
            inputs_q = torch.clamp(inputs_q, qmin, qmax)
            inputs_q = (inputs_q - new_zero_point) * new_scale
